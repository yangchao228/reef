...
**文档状态：** 当前可执行版本  
**更新日期：** 2026-03-27  
**关联文档：** `docs/design.md`、`docs/product-direction.md`、`docs/phased-roadmap.md`、`docs/ops-runbook.md`
...
- 后台补偿同步、失败分类、恢复动作和补偿批次摘要已可直接使用；
- 当前补偿脚本已具备 workspace 级并发保护和短窗口去重保护，避免重复执行同一轮补偿；
- 生产 scheduler 接入与运维 runbook 已明确，但仍需按实际部署环境完成最后挂载。
...
- 查看最近同步日志
- 查看补偿批次摘要、`compensationRunId` 与恢复动作
...
- 默认每 15 分钟对单个 `workspace` 执行一次 `--only-failed`
- 同一 scheduler 任务只负责一个 `workspace`
- 当前仓库已提供 `deploy/reef-compensate.sh` 作为宿主机调度包装脚本
- 若采用 Docker 部署，可直接通过 `docker compose run --rm compensator` 执行补偿

更完整的处理顺序见 `docs/ops-runbook.md`。
...
1. 继续完善同步补偿和失败恢复；
2. 把 scheduler 与运维 runbook 对应到真实部署环境；
3. 继续清理剩余单租户/过渡逻辑；
4. 继续扩大 CI 和 smoke 对后台主链的覆盖；
5. 主链稳定后，再推进 Agent Module。
